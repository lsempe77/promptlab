...
def get_or_create_baseline(conn: sqlite3.Connection, field_name: str) -> sqlite3.Row | None:
    ...
    """
    existing = db.best_accepted_prompt_version(conn, field_name)
    if existing:
        return existing
    instruction = prompts.BASELINE_INSTRUCTIONS[field_name]
    db.add_prompt_version(conn, field_name, version=1, template=instruction, parent_id=None, notes="baseline v1", accepted=1)
    return db.best_accepted_prompt_version(conn, field_name)


def add_version(
    conn: sqlite3.Connection, field_name: str, instruction: str, parent_id: int | None, notes: str | None,
    accepted: bool = False,
) -> sqlite3.Row | None:
    ...
    slot in the lineage.
    """
    latest = db.latest_prompt_version(conn, field_name)
    next_version = (latest["version"] + 1) if latest else 1
    db.add_prompt_version(
        conn, field_name, version=next_version, template=instruction, parent_id=parent_id, notes=notes,
        accepted=int(accepted),
    )
    return conn.execute(
        "SELECT * FROM prompt_versions WHERE field_name = ? AND version = ?", (field_name, next_version)
    ).fetchone()
